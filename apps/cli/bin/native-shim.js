...
const PLATFORM_PACKAGES = {
  ...
  "darwin-x64": "@beilin/runtime-darwin-x64",
  "darwin-arm64": "@beilin/runtime-darwin-arm64",
  "win32-x64": "@beilin/runtime-win32-x64",
};

function platformPackage(platform = process.platform, arch = process.arch) {
  const packageName = PLATFORM_PACKAGES[`${platform}-${arch}`];
  if (!packageName) {
    throw new Error(`Unsupported pcraft platform: ${platform}-${arch}`);
  }
  return packageName;
}
...
function resolveRuntime(env = process.env, resolver = require.resolve) {
  if (env.PCRAFT_BUNDLE_DIR) {
    return validateRuntime(env.PCRAFT_BUNDLE_DIR);
  }
  ...
  try {
    ...
    const message =
      `No pcraft runtime package found for ${process.platform}-${process.arch}.\n` +
      `Install with npm 7+ so optional dependencies are installed.`;
...
function validateRuntime(bundleDir) {
  const executable = path.join(bundleDir, "bin", binaryName("pcraft"));
  if (!fs.existsSync(executable)) {
    throw new Error(`pcraft native binary not found at ${executable}`);
  }
...
function run(argv, env = process.env) {
  ...
  try {
    ...
    console.error(`[pcraft] ${error.message}`);
    return 1;
  }
  ...
  const child = spawnSync(runtime.executable, argv, {
    ...
    env: {
      ...env,
      PCRAFT_BUNDLE_DIR: runtime.bundleDir,
    },
  });

  if (child.error) {
    console.error(`[pcraft] failed to launch native binary: ${child.error.message}`);
    return 1;
  }
  if (child.signal) {
    return 1;
