...
import { usePcraftRestart } from "./use-pcraft-restart";
...
describe("usePcraftRestart", () => {
  it("starts a restart and waits for a new boot id", async () => {
    ...
    const { result } = renderHook(() => usePcraftRestart({ onComplete }));
  ...
  it("keeps waiting while the backend is unreachable", async () => {
    ...
    const { result } = renderHook(() => usePcraftRestart());
