...
export type PcraftRestartPhase = "idle" | "starting" | "restarting" | "done" | "error";

const POLL_INTERVAL_MS = 2000;
const MAX_DURATION_MS = 3 * 60 * 1000;

type UsePcraftRestartArgs = {
  onComplete?: () => void;
};

export type PcraftRestartController = {
  phase: PcraftRestartPhase;
...
export function usePcraftRestart({
  onComplete,
}: UsePcraftRestartArgs = {}): PcraftRestartController {
  const [phase, setPhase] = useState<PcraftRestartPhase>("idle");
  ...
  const start = useCallback(async () => {
    ...
    try {
      ...
      fail(e instanceof Error ? e.message : "Failed to restart Pcraft");
    }
  }, [fail]);
