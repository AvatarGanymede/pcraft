...
export type BootstrapState =
  | { kind: "idle" }
  | { kind: "loading" }
  | { kind: "ready"; data: ImprovePcraftBootstrapResponse; steps: WorkflowStep[] }
...
function bootstrapBlockedReason(bootstrap: BootstrapState): string | null {
  if (bootstrap.kind === "loading" || bootstrap.kind === "idle") {
    return "Preparing pcraft repository…";
  }
  return null;
}
...
export function CreateModeView(props: CreateModeViewProps) {
  ...
  // user can't fork kdlbs/pcraft (e.g., EMU enterprise restriction). Showing
...
const STEP_DESCRIPTIONS: Record<string, string> = {
  ...
  test: "Agent boots a secondary pcraft instance with make dev and tells you what to verify. You confirm the change works in the running app.",
  pr: "Agent runs the pr skill: pushes the branch and opens a pull request to kdlbs/pcraft for the maintainers to review.",
};
        ...
        <div className="mt-2 space-y-3 rounded-md border border-border bg-muted/20 px-3 py-2 text-xs leading-relaxed text-muted-foreground">
          ...
          <div className="space-y-2">
            ...
            <UsefulInfoItem cmd="make install && make dev">
              Boots a secondary pcraft dev instance with a clean DB so you can verify the change
      ...
      <div className="flex items-center gap-2 text-sm text-muted-foreground">
        ...
        <TooltipProvider delayDuration={150}>
          <Tooltip>
            ...
            <TooltipContent side="top" className="max-w-xs text-xs leading-relaxed">
              Pcraft keeps a small in-memory ring buffer of the most recent backend logs and browser
  ...
  if (bootstrap.kind === "loading" || bootstrap.kind === "idle") {
    return (
      <div className="flex items-center gap-2 rounded-md border border-border bg-muted/30 px-3 py-2 text-xs text-muted-foreground">
        ...
        Preparing pcraft repository in background. Fill in the details, submit when ready.
      </div>
    );
  }
      ...
      <DialogContent className="sm:max-w-xl">
        ...
        <div className="space-y-4">
          ...
          <div className="flex justify-end">
            <Button
              data-testid="improve-pcraft-blocked-close"
              variant="outline"
...
function ContributorBanner({ data }: { data: ImprovePcraftBootstrapResponse }) {
  ...
  return (
    <div className="flex items-start gap-2 rounded-md border border-border bg-muted/30 px-3 py-2 text-xs text-muted-foreground">
      ...
      <span>
        ...
        .{" "}
        {hasWrite
          ? "You have write access to kdlbs/pcraft, so the agent will push directly to a branch on the upstream repo."
          : "The agent will fork kdlbs/pcraft to your account during the PR step and open a pull request from your fork."}
      </span>
    </div>
  );
}
