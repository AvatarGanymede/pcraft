...
import { bootstrapImprovePcraft } from "@/lib/api/domains/improve-pcraft-api";
...
import { CreateModeView, type BootstrapState } from "./improve-pcraft-dialog-create";

type ImprovePcraftDialogProps = {
  open: boolean;
...
export function ImprovePcraftDialog(props: ImprovePcraftDialogProps) {
  ...
  useBootstrapPcraft(mode, workspaceId, setBootstrap);
  ...
  const transformDescription = useCallback(
    async (description: string) => {
      ...
      return buildImprovePcraftDescription(description, bootstrap.data, captureLogs);
    },
...
function useBootstrapPcraft(
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const data = await bootstrapImprovePcraft(workspaceId);
        ...
        if (data.fork_status === "blocked_emu") {
          if (cancelled) return;
          const message = data.fork_message || "Your account cannot fork kdlbs/pcraft.";
        ...
        // Refresh the workspace repository list so the newly-created pcraft
        ...
        toast({
          title: "Could not prepare Improve Pcraft",
          description: message,
          variant: "error",
        });
      }
  ...
  return (
    <div className="space-y-5 py-2">
      <p className="text-sm leading-relaxed text-muted-foreground">
        Pcraft is open source, and you can help make it better.
      </p>

      <p className="text-sm leading-relaxed text-muted-foreground">
        Describe a bug you hit or a feature you&apos;d like, and we&apos;ll create a task on your
        own agent to implement it in the pcraft codebase.
      </p>

      <p className="text-sm leading-relaxed text-muted-foreground">
        When it&apos;s done, the agent opens a pull request to{" "}
        <code className="font-mono text-xs">kdlbs/pcraft</code> for the maintainers to review,
      ...
      <ul className="space-y-2 text-sm text-muted-foreground">
        ...
        <IntroBullet>Your agent implements it in the pcraft repository, with tests</IntroBullet>
        <IntroBullet>You verify and test the change in a second pcraft instance</IntroBullet>
        <IntroBullet>
          The agent forks <code className="font-mono text-xs">kdlbs/pcraft</code> to your GitHub
      ...
      <div className="flex justify-end gap-2">
        ...
        <Button
          ...
          data-testid="improve-pcraft-proceed"
        >
          Contribute
        </Button>
      </div>
