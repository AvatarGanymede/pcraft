...
import { extractPcraftStem, extractMcpResult } from "./pcraft/parse";
import { getPcraftRenderer } from "./pcraft/registry";
...
import { PcraftPermissionUIProvider, type PcraftPermissionUIState } from "./pcraft/shared";
...
): PcraftPermissionUIState {
  if (isPermissionPending) return "pending";
  if (permissionStatus === "rejected") return "rejected";
  return undefined;
}

type PcraftToolMessageProps = {
  comment: Message;
  permissionMessage?: Message;
};

// pcraftStemOf scans the several fields that may carry the raw MCP tool name
// and returns the first one that parses to a known pcraft stem. The fields
...
function pcraftStemOf(comment: Message): string | null {
  ...
  for (const candidate of candidates) {
    const stem = extractPcraftStem(candidate);
    if (stem) return stem;
  }
  return null;
}

// hasPcraftRenderer is the matcher used by the message dispatcher. It accepts
// any `tool_call` whose tool name is recognised as a Pcraft MCP tool AND for
// which a per-tool renderer is registered. We require both because we still
// want unregistered Pcraft tools to fall through to the generic ToolCallMessage
// (rather than rendering an empty row) until a dedicated renderer ships.
export function hasPcraftRenderer(comment: Message): boolean {
  if (comment.type !== "tool_call") return false;
  return !!getPcraftRenderer(pcraftStemOf(comment));
}

// PcraftToolMessage is the rendered entry point for every Pcraft tool call.
...
export const PcraftToolMessage = memo(function PcraftToolMessage({
  comment,
  permissionMessage,
}: PcraftToolMessageProps) {
  const meta = comment.metadata as ToolCallMetadata | undefined;
  const renderer = getPcraftRenderer(pcraftStemOf(comment));
  ...
  const rendered = (
    <PcraftPermissionUIProvider value={permissionUI}>
      {renderer({ args, result, status: meta?.status })}
    </PcraftPermissionUIProvider>
  );
  ...
  return (
    <>
      {rendered}
      <div className="mt-2 ml-7" data-testid="pcraft-tool-permission">
        <PermissionActionRow
