...
export type PcraftStatus = "pending" | "running" | "complete" | "error" | undefined;

export function PcraftStatusIcon({ status }: { status: PcraftStatus }) {
...
// it — see derivePermissionUI in pcraft-tool-message.tsx for the reasoning.
export type PcraftPermissionUIState = "pending" | "rejected" | undefined;

const PcraftPermissionUIContext = createContext<PcraftPermissionUIState>(undefined);

export const PcraftPermissionUIProvider = PcraftPermissionUIContext.Provider;
...
// PcraftRow is the consistent header row for every Pcraft tool. Title is the
// "Pcraft: …" string, summary is an inline description of args / result count,
// and children are the expanded body. Auto-expands while the tool is running,
// matching the pattern used by ToolExecuteMessage.
export function PcraftRow({
  Icon,
  title,
  summary,
  status,
  ...
  const permissionUI = useContext(PcraftPermissionUIContext);
...
function renderHeaderStatusIcon({
  ...
  return <PcraftStatusIcon status={status} />;
}

// PcraftBody is the bordered container used by all expanded sections so they
// match the look of ToolCallExpandedContent / ToolExecuteContent.
export function PcraftBody({ children }: { children: ReactNode }) {
...
> = {
  ...
  DONE: { variant: "default", className: "bg-green-600 hover:bg-green-600" },
