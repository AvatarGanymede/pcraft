...
// Constants identifying the canonical pcraft repository and the workflow
// template loaded from apps/backend/config/workflows/improve-pcraft.yml.
const (
	repoCloneURL  = "https://github.com/AvatarGanymede/pcraft"
	repoOwner     = "AvatarGanymede"
	repoName      = "pcraft"
	repoProvider  = "github"
	defaultBranch = "main"

	templateID   = "improve-pcraft"
	workflowName = "Improve Pcraft"
...
// Handler exposes the improve-pcraft HTTP endpoints.
type Handler struct {
...
func RegisterRoutes(router *gin.Engine, h *Handler) {
	...
	api := router.Group("/api/v1/system/improve-pcraft")
...
const (
	...
	// ForkStatusReady: user already has a fork at github.com/{login}/pcraft,
...
func (h *Handler) httpBootstrap(c *gin.Context) {
	...
	if err != nil {
		h.log.Error("improve-pcraft: repository upsert failed", zap.Error(err))
		c.JSON(http.StatusInternalServerError, gin.H{"error": "failed to register pcraft repository"})
		return
	}

	workflow, err := h.ensureWorkflow(ctx, req.WorkspaceID)
	if err != nil {
		h.log.Error("improve-pcraft: workflow upsert failed", zap.Error(err))
		c.JSON(http.StatusInternalServerError, gin.H{"error": "failed to ensure improve-pcraft workflow"})
		return
	}

	dir, err := createBundleDir()
	if err != nil {
		h.log.Error("improve-pcraft: bundle dir creation failed", zap.Error(err))
	...
	if err := writeMetadata(dir, h.version, nil); err != nil {
		h.log.Error("improve-pcraft: metadata write failed", zap.Error(err))
		c.JSON(http.StatusInternalServerError, gin.H{"error": "failed to write metadata"})
		return
	}
	if err := writeBackendLog(dir, h.snapshot()); err != nil {
		h.log.Error("improve-pcraft: backend log write failed", zap.Error(err))
...
// resolveOrCloneRepo returns the workspace's pcraft repository, preferring an
...
//  1. Match by provider info (workspace + github + kdlbs + pcraft).
//  2. Match by scanning workspace repos whose origin remote resolves to
//     kdlbs/pcraft; backfill provider info on the match.
...
func (h *Handler) resolveOrCloneRepo(ctx context.Context, workspaceID string) (*taskmodels.Repository, error) {
	...
	if err != nil {
		...
		h.log.Warn("improve-pcraft: list repositories failed; falling back to clone", zap.Error(err))
	} else if match := findPcraftRepoByLocalRemote(repos, h.resolveRemote); match != nil {
		h.backfillPcraftProviderInfo(ctx, match)
		return match, nil
	}
...
// findPcraftRepoByLocalRemote returns the first repo with a local path whose
// origin remote resolves to kdlbs/pcraft. Skips rows that already declare
// non-matching provider info to avoid hijacking unrelated entries.
func findPcraftRepoByLocalRemote(repos []*taskmodels.Repository, resolve remoteResolver) *taskmodels.Repository {
	if resolve == nil {
		return nil
	}
	for _, r := range repos {
...
// backfillPcraftProviderInfo fills missing provider/owner/name on an existing
...
func (h *Handler) backfillPcraftProviderInfo(ctx context.Context, repo *taskmodels.Repository) {
	...
	if _, err := h.taskSvc.UpdateRepository(ctx, repo.ID, &taskservice.UpdateRepositoryRequest{
		...
		h.log.Warn("improve-pcraft: backfill provider info failed",
...
const emuBlockedMessage = "Your GitHub account appears to be an Enterprise Managed User (EMU) account, " +
	...
	"The PR step would fail when forking kdlbs/pcraft. Contact your GitHub admin " +
...
func (h *Handler) resolveGitHubAccess(ctx context.Context) githubAccess {
	...
	if err != nil {
		h.log.Debug("improve-pcraft: gh login lookup failed", zap.Error(err))
		return out
	}
	out.login = login
	hasWrite, err := h.gh.HasRepoWriteAccess(ctx, repoOwner, repoName)
	if err != nil {
		h.log.Debug("improve-pcraft: gh write-access check failed", zap.Error(err))
		return out
	}
	...
	if err != nil {
		h.log.Debug("improve-pcraft: gh fork lookup failed", zap.Error(err))
		return out
	}
	if hasFork {
...
// ensureWorkflow finds or creates the hidden improve-pcraft workflow in the
		...
		if w.WorkflowTemplateID != nil && *w.WorkflowTemplateID == templateID {
			...
			if !w.Hidden {
				if err := h.taskSvc.SetWorkflowHidden(ctx, w.ID, true); err != nil {
					h.log.Warn("improve-pcraft: failed to heal hidden flag on stale record",
...
func (h *Handler) httpFrontendLog(c *gin.Context) {
	...
	if err := writeFrontendLog(dir, req.Entries); err != nil {
		h.log.Error("improve-pcraft: frontend log write failed", zap.Error(err))
