...
func TestFindPcraftRepoByLocalRemote(t *testing.T) {
	resolver := func(path string) (string, string, string) {
		switch path {
		case "/home/u/pcraft":
			return "github", "kdlbs", "pcraft"
		case "/home/u/fork":
			return "github", "alice", "pcraft"
		case "/home/u/other":
	...
	cases := []struct {
		...
		},
		{
			...
			repos: []*taskmodels.Repository{
				{ID: "match", LocalPath: "/home/u/pcraft"},
			},
			want: "match",
		},
		{
		...
		},
		{
			...
			repos: []*taskmodels.Repository{
				{ID: "wired", LocalPath: "/home/u/pcraft", Provider: "github", ProviderOwner: "someone", ProviderName: "pcraft"},
			},
			want: "",
		},
		{
			...
			repos: []*taskmodels.Repository{
				{ID: "first", LocalPath: "/home/u/pcraft"},
				{ID: "second", LocalPath: "/home/u/pcraft"},
			},
			want: "first",
		},
	}
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			got := findPcraftRepoByLocalRemote(tc.repos, resolver)
			if tc.want == "" {
				if got != nil {
...
func TestFindPcraftRepoByLocalRemote_NilResolver(t *testing.T) {
	repos := []*taskmodels.Repository{{ID: "x", LocalPath: "/p"}}
	if got := findPcraftRepoByLocalRemote(repos, nil); got != nil {
