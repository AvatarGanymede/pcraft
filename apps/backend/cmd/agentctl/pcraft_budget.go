...
// runBudgetCmd dispatches `agentctl pcraft budget <subcmd>`. The
...
func runBudgetCmd(args []string) int {
	if len(args) == 0 {
		cliError("usage: agentctl pcraft budget <get> [flags]")
		return 1
	}
	switch args[0] {
	case subcmdGet:
...
func budgetGet(args []string) int {
	...
	if *agentID == "" {
		return getWithParams(
			...
			"PCRAFT_WORKSPACE_ID", wsID, nil)
	}
	return getWithParams(
		fmt.Sprintf("/api/v1/office/workspaces/%s/costs/by-agent", wsID),
		"PCRAFT_WORKSPACE_ID", wsID, map[string]string{"agent_id": *agentID})
}
