...
// runRoutinesCmd dispatches `agentctl pcraft routines <subcmd>`.
...
func runRoutinesCmd(args []string) int {
	if len(args) == 0 {
		cliError("usage: agentctl pcraft routines <list|create|pause|resume|delete> [flags]")
		return 1
	}
	switch args[0] {
	case subcmdList:
...
func routinesList(args []string) int {
	...
	return getWithParams(path, "PCRAFT_WORKSPACE_ID", wsID, nil)
}
...
func routinesCreate(args []string) int {
	...
	wsID := os.Getenv("PCRAFT_WORKSPACE_ID")
	if wsID == "" {
		cliError("PCRAFT_WORKSPACE_ID must be set")
		return 1
	}
