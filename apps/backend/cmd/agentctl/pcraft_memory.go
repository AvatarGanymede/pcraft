...
func runMemoryCmd(args []string) int {
	if len(args) == 0 {
		cliError("usage: agentctl pcraft memory <get|set|summary> [flags]")
		return 1
	}
	switch args[0] {
	case subcmdGet:
...
func memoryGet(args []string) int {
	...
	return getWithParams(path, "PCRAFT_AGENT_ID", agentID, map[string]string{
		"layer": *layerFlag,
		"key":   *keyFlag,
	})
}
...
func memorySet(args []string) int {
	...
	if client.agentID == "" {
		cliError("PCRAFT_AGENT_ID must be set")
		return 1
	}
