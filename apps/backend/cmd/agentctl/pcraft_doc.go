...
func runDocCmd(args []string) int {
	if len(args) == 0 {
		cliError("usage: agentctl pcraft doc <create|read|list|upload> [flags]")
		return 1
	}
	switch args[0] {
	case subcmdCreate:
...
// Usage: pcraft doc create <task-id> <key> --type <type> --title <title> --content <content>
func docCreate(args []string) int {
	...
	if len(args) < 2 {
		cliError("usage: pcraft doc create <task-id> <key> [flags]")
		return 1
	}
	taskID := args[0]
	key := args[1]
...
// Usage: pcraft doc read <task-id> <key>
func docRead(args []string) int {
	...
	positional := fs.Args()
	if len(positional) < 2 {
		cliError("usage: pcraft doc read <task-id> <key>")
		return 1
	}
	taskID := positional[0]
	key := positional[1]
...
func docList(args []string) int {
	...
	positional := fs.Args()
	if len(positional) < 1 {
		cliError("usage: pcraft doc list <task-id>")
		return 1
	}
	taskID := positional[0]
...
// Usage: pcraft doc upload <task-id> <key> <filepath>
func docUpload(args []string) int {
	...
	positional := fs.Args()
	if len(positional) < 3 {
		cliError("usage: pcraft doc upload <task-id> <key> <filepath>")
		return 1
	}
	taskID := positional[0]
	key := positional[1]
