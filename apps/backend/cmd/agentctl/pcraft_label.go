...
func runLabelCmd(args []string) int {
	if len(args) == 0 {
		cliError("usage: agentctl pcraft label <add|remove|list> [flags]")
		return 1
	}
	switch args[0] {
	case "add":
...
func labelAdd(args []string) int {
	...
	taskFlag := fs.String("task", "", "Task ID (defaults to $PCRAFT_TASK_ID)")
	...
	if taskID == "" {
		cliError("task ID required: use --task or set PCRAFT_TASK_ID")
		return 1
	}
	if *nameFlag == "" {
