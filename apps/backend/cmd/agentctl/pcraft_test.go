...
func setEnvVars(t *testing.T, srv *httptest.Server) {
	t.Helper()
	t.Setenv("PCRAFT_API_URL", srv.URL)
	t.Setenv("PCRAFT_API_KEY", "test-key-123")
	t.Setenv("PCRAFT_RUN_ID", "run-456")
	t.Setenv("PCRAFT_AGENT_ID", "agent-789")
	t.Setenv("PCRAFT_TASK_ID", "task-abc")
	t.Setenv("PCRAFT_WORKSPACE_ID", "ws-def")
}
...
func TestTaskGet_CallsCorrectEndpoint(t *testing.T) {
	...
	code := runPcraftCLI([]string{"task", "get"})
	if code != 0 {
...
func TestTaskGet_ExplicitID(t *testing.T) {
	...
	code := runPcraftCLI([]string{"task", "get", "--id", "explicit-1"})
	if code != 0 {
		t.Fatalf("expected exit 0, got %d", code)
	}
...
func TestTaskUpdate_EmptyPayload_ReturnsError(t *testing.T) {
	...
	code := runPcraftCLI([]string{"task", "update"})
	if code == 0 {
...
func TestTaskCreate_NoExecutionPolicyOrBlockedBy_OmitsFields(t *testing.T) {
	...
	code := runPcraftCLI([]string{"task", "create", "--title", "Plain task"})
	if code != 0 {
		t.Fatalf("expected exit 0, got %d", code)
	}
...
// flags that moved off the kanban create_task_pcraft MCP tool and onto this
...
func TestCommentAdd_StdinMode(t *testing.T) {
	...
	code := runPcraftCLI([]string{"comment", "add", "--body", "-"})
	if code != 0 {
		t.Fatalf("expected exit 0, got %d", code)
	}
...
func TestCommentList_GetsCommentsWithLimit(t *testing.T) {
	...
	code := runPcraftCLI([]string{"comment", "list", "--limit", "5"})
	if code != 0 {
		t.Fatalf("expected exit 0, got %d", code)
	}
	if captured.Method != "GET" {
...
func TestAgentsList_FiltersRoleAndStatus(t *testing.T) {
	...
	code := runPcraftCLI([]string{"agents", "list", "--role", "worker", "--status", "idle"})
	if code != 0 {
		t.Fatalf("expected exit 0, got %d", code)
	}
...
func TestAgentsList_SpecialCharsInFilterAreURLEncoded(t *testing.T) {
	...
	code := runPcraftCLI([]string{"agents", "list", "--role", "a&b=c", "--status", "x y"})
	if code != 0 {
		t.Fatalf("expected exit 0, got %d", code)
	}
...
// `pcraft doc create <task-id> <key> --type ... --title ... --content ...`,
...
func TestMemoryGet_QueriesByLayer(t *testing.T) {
	...
	code := runPcraftCLI([]string{"memory", "get", "--layer", "facts"})
	if code != 0 {
		t.Fatalf("expected exit 0, got %d", code)
	}
	if captured.Path != "/api/v1/office/agents/agent-789/memory" {
...
func TestMemorySummary_CallsCorrectEndpoint(t *testing.T) {
	...
	code := runPcraftCLI([]string{"memory", "summary"})
	if code != 0 {
		t.Fatalf("expected exit 0, got %d", code)
	}
...
func TestCheckout_CallsEndpoint(t *testing.T) {
	...
	code := runPcraftCLI([]string{"checkout"})
	if code != 0 {
		t.Fatalf("expected exit 0, got %d", code)
	}
	if captured.Method != "POST" {
...
func TestMissingEnv_ReturnsClearError(t *testing.T) {
	...
	t.Setenv("PCRAFT_API_URL", "")
	t.Setenv("PCRAFT_API_KEY", "")

	code := runPcraftCLI([]string{"task", "get", "--id", "some-task"})
	if code != 1 {
...
func TestDefaultTaskID_UsesEnvVar(t *testing.T) {
	srv, captured := setupMockServer(t, 200, `{"task":{"id":"task-abc"}}`)
	setEnvVars(t, srv)
	// Do not pass --id, should use PCRAFT_TASK_ID.
	code := runPcraftCLI([]string{"task", "get"})
	if code != 0 {
		t.Fatalf("expected exit 0, got %d", code)
	}
...
func TestUnknownCommand_ReturnsError(t *testing.T) {
	code := runPcraftCLI([]string{"invalid"})
	if code != 1 {
		t.Fatalf("expected exit 1, got %d", code)
	}
}
...
func TestRunIDHeader_NotSetOnGet(t *testing.T) {
	...
	runPcraftCLI([]string{"task", "get"})
	if captured.Header.Get("X-Pcraft-Run-Id") != "" {
		t.Error("X-Pcraft-Run-Id should not be set on GET requests")
	}
}
...
func TestMissingAgentID_ForMemory(t *testing.T) {
	...
	setEnvVars(t, srv)
	t.Setenv("PCRAFT_AGENT_ID", "")

	code := runPcraftCLI([]string{"memory", "get"})
	if code != 1 {
		t.Fatalf("expected exit 1, got %d", code)
	}
}
...
func TestMissingWorkspaceID_ForAgents(t *testing.T) {
	srv, _ := setupMockServer(t, 200, `{}`)
	setEnvVars(t, srv)
	t.Setenv("PCRAFT_WORKSPACE_ID", "")

	code := runPcraftCLI([]string{"agents", "list"})
	if code != 1 {
		t.Fatalf("expected exit 1, got %d", code)
	}
}
...
func assertRunIDHeader(t *testing.T, captured *capturedRequest, expected string) {
	t.Helper()
	runID := captured.Header.Get("X-Pcraft-Run-Id")
	if runID != expected {
		t.Errorf("expected X-Pcraft-Run-Id=%s, got %s", expected, runID)
	}
}
