...
// runPcraftCLI dispatches the pcraft subcommand to the appropriate handler.
// Returns an exit code (0 = success, non-zero = error).
func runPcraftCLI(args []string) int {
	if len(args) == 0 {
		printUsage()
		return 1
	}
...
func printUsage() {
	fmt.Fprintln(os.Stderr, "Usage: agentctl pcraft <command> [flags]")
