...
func runTaskCmd(args []string) int {
	if len(args) == 0 {
		cliError("usage: agentctl pcraft task <get|update|create> [flags]")
		return 1
	}
	switch args[0] {
	case subcmdGet:
...
// taskGet fetches a task by ID. Defaults to $PCRAFT_TASK_ID when --id is omitted.
func taskGet(args []string) int {
	fs := flag.NewFlagSet("task get", flag.ContinueOnError)
	id := fs.String("id", "", "Task ID (defaults to $PCRAFT_TASK_ID)")
	...
	if taskID == "" {
		cliError("task ID required: use --id or set PCRAFT_TASK_ID")
		return 1
	}
...
// the default (from PCRAFT_TASK_ID env var).
