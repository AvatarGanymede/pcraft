...
// runTasksCmd dispatches `agentctl pcraft tasks <subcmd>`. The
// singular `agentctl pcraft task <subcmd>` group (get/update/create)
...
func runTasksCmd(args []string) int {
	if len(args) == 0 {
		cliError("usage: agentctl pcraft tasks <list|move|archive|message|conversation> [flags]")
		return 1
	}
	switch args[0] {
	case subcmdList:
...
func tasksList(args []string) int {
	...
	wsID := os.Getenv("PCRAFT_WORKSPACE_ID")
	path := fmt.Sprintf("/api/v1/office/workspaces/%s/tasks", wsID)
	return getWithParams(path, "PCRAFT_WORKSPACE_ID", wsID, map[string]string{
		"status":   *status,
		"assignee": *assignee,
		"project":  *project,
	})
...
func tasksMove(args []string) int {
	...
	id := fs.String("id", "", "Task ID (required; defaults to $PCRAFT_TASK_ID)")
	...
	if taskID == "" || *step == "" {
		cliError("--id (or PCRAFT_TASK_ID) and --step are required")
		return 1
	}
...
func tasksArchive(args []string) int {
	...
	taskID := resolveTaskID(*id, client.taskID)
	if taskID == "" {
		cliError("--id (or PCRAFT_TASK_ID) required")
		return 1
	}
	body, status, err := client.do(http.MethodPost,
...
func tasksMessage(args []string) int {
	...
	taskID := resolveTaskID(*id, client.taskID)
	if taskID == "" || *prompt == "" {
		cliError("--id (or PCRAFT_TASK_ID) and --prompt are required")
		return 1
	}
